...
class Embeds(commands.Cog):
  """Cog for sending static server embeds like rules and ToS."""

  def __init__(self, bot):
    self.bot = bot

  #Rules Message
  @commands.hybrid_command(name="rules", description="Sends the rules embed to a channel.")
  ...
  async def rules(self, ctx, channel: discord.TextChannel):
      ...
      try:
          ...
          await ctx.send(f"Rules message sent to {channel.mention}!", ephemeral=True)
      except discord.Forbidden:
          await ctx.send(f"I don't have permission to send messages in {channel.mention}.", ephemeral=True)

  #Terms of Service Message
  @commands.hybrid_command(name="tos", description="Sends the Terms of Service embed to a channel.")
  @commands.has_permissions(administrator=True)
  async def tos(self, ctx, channel: discord.TextChannel):
    
      # --- Refactor ---
      # Instead of sending 8 messages, we'll build a list of embeds
      # and send them all in one message.
